...
import type { DriverInfo, Location, PaymentMethod, VehicleType } from '@/types';
import type { RideHistoryItem, RideHistoryStatus } from '@/features/customer/types';
import { fetchCustomerAccount } from '@/services/accounts';
...
export async function cancelOrder(orderId: string): Promise<{ errorMessage: string | null }> {
  const { data, error } = await insforge.database
    .from('orders')
    .update({
      status: 'cancelled',
  ...
  if (error || !data) {
    return { errorMessage: error?.message ?? 'Could not cancel the ride.' };
  }
