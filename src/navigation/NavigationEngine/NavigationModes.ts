...
export enum NavigationMode {
  /** User selecting locations. No trip requested yet, and a Driver in this mode is available but unassigned. */
  IDLE = 'IDLE',
  ...
  /** Driver waiting for Customer. The Driver has reached the pickup point and is waiting to start the trip. */
  ARRIVED_PICKUP = 'ARRIVED_PICKUP',
  /** Customer onboard. The Driver is en route to the destination with the Customer/delivery aboard. */
  TRIP_IN_PROGRESS = 'TRIP_IN_PROGRESS',
  /** Driver reached destination. The Driver has arrived at the drop-off point; the trip is not yet finalized. */
  ARRIVED_DROPOFF = 'ARRIVED_DROPOFF',
  /** Trip finished. The trip has been finalized; a summary is shown before returning to IDLE. */
  TRIP_COMPLETED = 'TRIP_COMPLETED',
  /** Driver unavailable. The Driver is not online and cannot receive or navigate any request. */
  OFFLINE = 'OFFLINE',
}
...
export const NAVIGATION_MODE_TRANSITIONS: Readonly<Record<NavigationMode, readonly NavigationMode[]>> = {
  ...
  // booking. A Driver who is idle (unassigned, no active request) can
  ...
  // Booking confirmed -> broadcast to Drivers and wait for a match.
  // Or the Customer backs out before booking -> back to IDLE.
  [NavigationMode.PREVIEW]: [NavigationMode.MATCHING, NavigationMode.IDLE],

  // A Driver accepts -> they start navigating to the pickup point.
  // Or no Driver accepts (timeout) / the Customer cancels while
  // waiting -> back to IDLE.
  [NavigationMode.MATCHING]: [NavigationMode.DRIVER_TO_PICKUP, NavigationMode.IDLE],

  // The Driver reaches the pickup point -> now waiting for the
  ...
  // A Driver coming back online returns to IDLE (available, unassigned).
  [NavigationMode.OFFLINE]: [NavigationMode.IDLE],
};

/** Modes where a Driver is actively driving toward a destination (used to gate driving-only behaviour in later phases, e.g. GPS/camera). */
 ...
 * `OFFLINE -> IDLE` (a Driver coming back online) — both also route
